...
pub fn get_glfw_required_instance_extensions(
    glfw: &glfw::Glfw,
) -> Option<vulkano::instance::InstanceExtensions> {
    let exts = glfw.get_required_instance_extensions();
    match exts {
        Some(exts) => Some(vulkano::instance::InstanceExtensions::from_iter(
            exts.iter().map(AsRef::as_ref),
        )),
        None => None,
    }
}
...
fn main() {
    ...
    // a window.
    let required_extensions = get_glfw_required_instance_extensions(&glfw).unwrap();
    ...
    let surface = Arc::new({
        ...
        unsafe {
            Surface::from_handle(
                instance.clone(),
                surface.assume_init(),
                vulkano::swapchain::SurfaceApi::try_from(window.raw_window_handle()).unwrap(),
                None,
            )
        }
    });
